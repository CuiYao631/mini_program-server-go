...
func (tq *TagQuery) QueryResources() *ResourcesQuery {
	query := &ResourcesQuery{config: tq.config}
...
func (tq *TagQuery) sqlAll(ctx context.Context) ([]*Tag, error) {
	...
	if query := tq.withResources; query != nil {
		...
		for _, node := range nodes {
			...
			node.Edges.Resources = []*Resources{}
		}
		var (
			edgeids []string
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(tag.ResourcesPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, tq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "resources": %w`, err)
		}
		query.Where(resources.IDIn(edgeids...))
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "resources" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Resources = append(nodes[i].Edges.Resources, n)
			}
		}
	}
